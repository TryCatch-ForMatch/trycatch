...
// ---------------------------------------------------------------------------
integration(
  'GET /api/portfolio/[username] — portfólio público completo',
  () => {
    it('retorna 200 com todos os campos quando todos os toggles estão ativos', async () => {
      const res = await GET(
        makeRequest('joao_public'),
        makeParams('joao_public')
      );
    ...
    it('agrupa múltiplas stacks do mesmo projeto em um único item', async () => {
      ...
      const projects: Array<{ projectId: string; stacks: unknown[] }> =
        body.data.projects;
...
// ---------------------------------------------------------------------------
integration('GET /api/portfolio/[username] — campos ocultos por toggle', () => {
  it('retorna 200 mas sem email quando showEmail = false', async () => {
    const res = await GET(
      makeRequest('maria_parcial'),
      makeParams('maria_parcial')
    );
    const body = await res.json();
...
// ---------------------------------------------------------------------------
integration('GET /api/portfolio/[username] — portfólio privado', () => {
  it('retorna 404 quando portfolioPublic = false', async () => {
    const res = await GET(
      makeRequest('carlos_private'),
      makeParams('carlos_private')
    );
...
// ---------------------------------------------------------------------------
integration('GET /api/portfolio/[username] — usuário inativo', () => {
  it('retorna 404 quando isActive = false', async () => {
    const res = await GET(
      makeRequest('ana_inactive'),
      makeParams('ana_inactive')
    );
...
// ---------------------------------------------------------------------------
integration('GET /api/portfolio/[username] — usuário inexistente', () => {
  it('retorna 404 para username que não existe', async () => {
    const res = await GET(
      makeRequest('usuario_que_nao_existe_xyz'),
      makeParams('usuario_que_nao_existe_xyz')
    );
