...
import {
  Eye, Globe, GlobeLock, Save, ExternalLink,
  Github, Linkedin, Mail, User,
} from 'lucide-react';
import { SettingsCard, SettingsCardHeader } from '@/components/ui/settings-card';
...
export default function DashboardPortfolioClient() {
  const { form, username, isLoading, isSaving, saveStatus, onSubmit } = usePortfolioSettings();
  ...
  return (
    <main className="container mx-auto px-4 py-10 max-w-2xl space-y-8">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold tracking-tight">Meu portfólio</h1>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <SettingsCard>
          ...
          <ToggleField
            ...
            onChange={(v) => setValue('portfolioPublic', v, { shouldDirty: true })}
          />
        </SettingsCard>

        {/* ── Dados pessoais ────────────────────────────────────────── */}
        <SettingsCard>
          <SettingsCardHeader icon={<User className="w-4 h-4" />} title="Dados pessoais" />
        ...
        <div className="flex items-center justify-between pt-2">
          <div className="text-sm">
            {saveStatus === 'success' && (
              <span className="text-green-600 dark:text-green-400">✓ Alterações salvas</span>
            )}
            {saveStatus === 'error' && (
